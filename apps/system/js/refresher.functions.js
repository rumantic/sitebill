var LinkedElements = {
    refresh: function (el, linked_el_id, linked_field, model_name) {
        ...
        if (value != '') {
            var data = {
                action: 'get_options',
                frommodelfield: linked_el_id,
                byfield: linked_field,
                value: value,
                model: model_name
            };
            if (!is_connected_element_select) {
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (typeof connected_element.selectpicker !== "undefined") {
                        connected_element.selectpicker('destroy');
                    }
                    ...
                    if (typeof connected_element.selectpicker !== "undefined") {
                        connected_element.selectpicker();
                    }
    ...
    setHandler: function (el_class, connected_el_class, linked_field) {
        ...
        _this.change(function () {
            ...
            $.ajax({
                ...
                data: {
                    action: 'get_options',
                    frommodelfield: connected_modelfield,
                    byfield: linked_field,
                    value: value
                },
                dataType: 'json',
...
$(document).ready(function () {
    ...
    $('.multiselect_block').each(function () {
        const multiselectblock = $(this);
        const autocomplete = multiselectblock.find('.multiselect-autocomplete').eq(0);
        const holder = multiselectblock.find('.multiselect-holder').eq(0);

        let existing = [];
        holder.find('.multiselect-item').each(function (n, i) {
            existing.push($(i).find('input').val());
        });
        const model = autocomplete.data('model');
        const element = autocomplete.data('element');

        $(document).on('click', '.multiselect-holder .multiselect-item', function () {
            $(this).remove();
        })

        autocomplete.autocomplete({
            open: function () {
                $('.ui-menu').width(autocomplete.width());
            },
            ...
            select: function (event, ui) {
                ...
                autocomplete.val('');
                holder.append($('<span class="multiselect-item"><input type="hidden" name="' + element + '[]" value="' + ui.item.value + '">' + ui.item.label + '<button type="button" class="close">×</button></span>'));
                return false;
            }
        })
    });
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        ...
                        $.map(json, function (n, i) {
                            ...
                            answer.push({
                                id: n[_pk],
                                value: n[_default_field],
                                label: n[_default_field]
                            });
                        });
                        response(answer);
                    }
