DataImagelist = {
    ...
    dz_dblClick: function (el, pk_value, table, pk_name, field_name) {
        ...
        var all_els = parent.find('.dz-preview-uploaded-item-description-editable');
        var current_element_index =  all_els.index(description_editable_block);
        //return false;
...
function s3Update(response, dropzone) {
    const data = {
        action: 'sharder',
        do: 'add_item',
        element: dropzone.options.customparams.element,
        table: dropzone.options.customparams.model,
        primary_key: dropzone.options.customparams.primary_key,
        primary_key_value: dropzone.options.customparams.primary_key_value,
