var ActiveMap = {
            ...
            for (var i in this.sorts) {
                if (this.sorts[i].prop('tagName').toLowerCase() == 'select') {
                    this.sorts[i].change(function (e) {
                        ...
                        if (s.data('order') != undefined) {
                            search_params.order = s.data('order');
                        }
                        if (s.data('sort') != undefined) {
    ...
    decorateItem: function (data) {
        ...
        block.find('.ActiveMapListBlock-item-price').text(data.price + (data.currency_name ? ' ' + data.currency_name : ''));
        ...
        for (var i in json.paging.pages) {
            if (json.paging.pages[i].text >= left && json.paging.pages[i].text <= right) {
                ...
                x.click(function (e) {
                    ...
                    _this.reloadListingData();
                    if (typeof _this.listing != 'undefined') {
                        $([document.documentElement, document.body]).animate({
                            scrollTop: _this.listing.offset().top
                        }, 500);
                    }
                });
                pg.append(x);
        ...
        return function () {
            return {
                ...
                clearMap: function () {
                    ...
                    if (this.parent.markers.length > 0) {
                        this.parent.markers.map(function (m) {
                            m.setMap(null)
                        });
                        this.parent.markers = [];
                ...
                buildMap: function () {
                    ...
                    this.parent.clusterer = new MarkerClusterer(this.parent.map, [], {
                        gridSize: 50,
                        maxZoom: 15,
                        imagePath: estate_folder + '/apps/third/google/markerclusterer/images/m'
                    });
