...
class Menu_Structure_Manager extends Object_Manager {
    ...
    function grid1 ( $menu_id ) {
        ...
        while ( $this->db->fetch_assoc() ) {
            ...
            $rs .= '<tr>';
            $rs .= '<td class="'.$row_class.'" nowrap width="99%">'.$this->db->row[$this->grid_key];
            $rs .= '</td>';
            $rs .= '<td class="'.$row_class.'" nowrap ">'.$this->db->row['url'].'</td>';
            
            $rs .= '<td width="10%" nowrap style="vertical-align: top;">
            <a href="?action=menu&do=structure&menu_id='.$menu_id.'&subdo=edit&'.$this->primary_key.'='.$this->db->row[$this->primary_key].'"><img src="'.SITEBILL_MAIN_URL.'/img/edit.gif" border="0"></a> 
            <a href="?action=menu&do=structure&menu_id='.$menu_id.'&subdo=delete&'.$this->primary_key.'='.$this->db->row[$this->primary_key].'" onclick="if ( confirm(\''.Multilanguage::_('L_MESSAGE_REALLY_WANT_DELETE').'\') ) {return true;} else {return false;}"><img src="'.SITEBILL_MAIN_URL.'/img/delete.gif"></a>
            </td>';
            $rs .= '</tr>';
        }
        $rs .= '</table></div>';
