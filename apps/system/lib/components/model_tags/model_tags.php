...
class model_tags extends ajax_common
{
    ...
    function init_session_tags()
    {
        ...
        if ($model_name != '') {
            if ( $column_name != '' ) {
                $_SESSION['model_tags'][$model_name]['tags_array'][$column_name] = $tags_array[$column_name];
            } else {
    ...
    function get_array_by_query($item_array, $model = null)
    {
        ...
        if (
            is_array($item_array['parameters']) &&
            @$item_array['parameters']['api_query'] &&
            $this->request()->get('version') == 'api'
        ) {
            $query = $item_array['parameters']['api_query'];
        }
