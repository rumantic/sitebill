...
{
    use \system\traits\statistics\StatisticsTrait;

    private $context;
    private $excel_free_admin;
    /**
     * @var dropzone_model_adapter
     */
    ...
    function ajax()
    {
        //$this->writeLog(var_export($_REQUEST, true));
        $excel_free_admin = false;
        if ($this->getRequestValue('model_name') == 'data') {
            if (file_exists(SITEBILL_DOCUMENT_ROOT . '/apps/excel/admin/admin.php')) {
                require_once(SITEBILL_DOCUMENT_ROOT . '/apps/excel/admin/admin.php');
                require_once SITEBILL_DOCUMENT_ROOT . '/apps/excel/admin/data_manager_export.php';
                $excel_free_admin = new excel_admin();
                $excel_admin_tag = 'excel_admin';
            } else {
                ...
                $excel_admin_tag = 'excelfree_admin';
            }
        }
        ...
        if ($this->getRequestValue('do') == 'delete') {
            ...
            if ($excel_free_admin) {
                $html_table = $excel_free_admin->xls_parser();
            } else {
            ...
            //exit;
        } elseif ($this->getRequestValue('do') == 'parse_xls_json') {
            $file_name = $this->getRequestValue('file_name');

            if ( !empty($file_name) ) {
                $excel_full_data = $this->xls_parser_json($file_name);
            } else {
                $this->riseError('Error on loading file: empty file_name');
            }
            if ( $this->getError() ) {
                $response = array(
                    'status' => 'error',
                ...
                $response = array(
                    'status' => 'OK',
                    'excel_full_data' => $excel_full_data,
                    'file_name' => $file_name,
                );
            }
            return $response;
            ...
            if ($excel_free_admin) {
                ...
                //ini_set('error_log', SITEBILL_DOCUMENT_ROOT.'/w.log');
                $file_name = $this->getRequestValue('file_name');
                ob_start();
                $content = $excel_free_admin->run_import($file_name);
                ob_end_clean();
                //$content = 'test';
            } else {
                ob_start();
                $content = $this->run_import();
                ob_end_clean();
            }
            //$this->writeLog('content111 = '. $content);

            echo json_encode(array('status' => 'OK', 'content' => $content));
            exit;
        } elseif ($this->getRequestValue('do') == 'import_json') {
            return $this->run_import_json();
        } else {
            $file_mode = 'excel';
            if (!empty($_FILES)) {
    ...
    function run_import_json()
    {
        $page = $this->request()->get('page');
        $page++;
        $import_status = 'import_in_progress';

        if ( $page >= $this->request()->get('totalPages') ) {
            $import_status = 'import_complete';
        }

        try {
            $import_result = $this->run_import_per_page(
                $this->request()->get('mapped_columns'),
                $this->request()->get('excel_header'),
                $this->getConfigValue('apps.excelfree.rows_per_page'),
                $page,
                $this->request()->get('totalPages')
            );
            $data = [
                'import_result' => $import_result,
                'page' => $page,
                'import_status' => $import_status,
                'import_statistics' => $this->get_all_stat(),
            ];
            $response = new \api\aliases\API_response_alias('OK', 'success', $data);
        } catch (Exception $e) {
            $response = new \api\aliases\API_response_alias('ERROR', $e->getMessage(), []);
        }

        return $response->get();
    }

    function run_import_per_page($assoc_array, $excel_header, $limit, $page, $totalPages)
    {
        if ( !is_array($assoc_array) and count($assoc_array) < 1 ) {
            throw new Exception('assoc_array not defined');
        }
        ...
        $file_name = $this->getRequestValue('file_name');
        if ($file_name) {
            $data = $excel_free_admin->load_xls($file_name, false, $limit, $page);

            //$this->writeLog(__METHOD__.', data b = <pre>'.var_export($data, true).'</pre>');
            $data = $this->remapping_by_excel_header($data, $excel_header);
            //$this->writeLog(__METHOD__.', data a = <pre>'.var_export($data, true).'</pre>');
            //$this->writeLog(__METHOD__.', assoc = <pre>'.var_export($assoc_array, true).'</pre>');

            //$this->writeLog(__METHOD__.', import data = <pre>'.var_export($data, true).'</pre>');


            $this->sql_exec($this->get_table_name(), $data, $mapper, $assoc_array, $page);
            if ( $page >= $totalPages ) {
     ...
     * @return string
     */
    function sql_exec($table_name, $data, $mapper, $assoc_array, $page = 1)
    {
        ...
        //$this->writeLog(__METHOD__.' exec mapper <pre>'.var_export($mapper, true).'</pre>');
        //$this->writeLog(__METHOD__.' exec assoc <pre>'.var_export($assoc_array, true).'</pre>');
        ...
        foreach ($data as $data_id => $data_array) {
            ...
            if ( !empty($data_array[$primary_key]) ) {
                $primary_key_value = $data_array[$primary_key];
            } else {
                $primary_key_value = $data_array[$assoc_array[$primary_key]];
            }
            ...
            if ($this->data_manager_export->is_record_exist($data_array, $assoc_array)) {
                ...
                if ($this->need_check_access($table_name)) {
                    if (!$this->check_access($table_name, $this->get_check_access_user_id($table_name), 'edit', $primary_key, $data_array[$primary_key])) {
                        $error_message = 'ID = ' . $data_array[$primary_key] . ', ' . Multilanguage::_('L_ACCESS_DENIED') . '<br>';
                        $rs .= $error_message;
                        $this->add_stat_error($error_message);
                        $this->increment_stat(self::$rejected_stat_items);
                    } else {
                        $rs .= $this->data_manager_export->edit();
                        if ( $this->data_manager_export->getError() ) {
                            $this->add_stat_error($this->data_manager_export->getError());
                            $this->increment_stat(self::$rejected_stat_items);
                        } else {
                            $this->increment_stat(self::$updated_stat_items);
                        }
                    }
                } else {
                    $rs .= $this->data_manager_export->edit();
                ...
                if ( $this->data_manager_export->getError() ) {
                    ...
                    $this->increment_stat(self::$new_stat_items);
                }
    ...
    function xls_parser_json ( $file_name )
    {
        require_once(SITEBILL_DOCUMENT_ROOT . '/apps/excelfree/admin/admin.php');
        $excel_free_admin = new excelfree_admin();
        if ($file_name != '') {
            $data = $excel_free_admin->load_xls($file_name, true, 20);
            if ( $this->getError() ) {
    ...
    function xls_parser($file_name)
    {
        ...
        if ($file_name != '') {
            ...
            //$this->writeLog('assoc = <pre>'.var_export($assoc_array, true).'</pre>');
    ...
    function remapping_by_excel_header($data, $header)
    {
        foreach ($data as $item_id => $item_a) {
            foreach ($item_a as $letter => $value) {
                $title = SiteBill::iconv('utf-8', SITE_ENCODING, $header[$letter]);
