...
trait MobilePhoneTrait {
    ...
    function compile_mobilephone_element($item_array)
    {
        ...
        $id = 'frm_'.md5($item_array['name'] . '_' . rand(100, 999));
        $string = '';
        ...
        if ( $this->getConfigValue('apps.realty.mobilephone_old_mask') ) {
            $html = '<input type="text" class="' . $this->classes['input'] . '" id="' . $id . '" placeholder="' . $item_array['title'] . '" name="' . $item_array['name'] . '" value="' . $value . '">';
        }else{
            $html = '
            ...
            <input id="' . $id . '" class="' . $this->classes['input'] . '" type="hidden" name="' . $item_array['name'] . '" value="' . $value . '" />';
        }
    ...
    function get_mobilephone_input_init_script_intl_tel($id, $item_array, $initialcountry = 'auto') {
    ...
    function get_mobilephone_input_init_script ($id, $item_array) {
        if ( $this->getConfigValue('apps.realty.mobilephone_old_mask') ) {
            ...
            $initialcountry = 'auto';
            if(isset($_SESSION['user_geoip']) && isset($_SESSION['user_geoip']['country']) && $_SESSION['user_geoip']['country'] != ''){
                $initialcountry = $_SESSION['user_geoip']['country'];
            }
            return $this->get_mobilephone_input_init_script_intl_tel($id, $item_array, $initialcountry);
        }
    }
}
