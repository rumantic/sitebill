...
class Register extends Login
{
        ...
        switch ($do) {
            ...
            case 'register':
                ...
                if (!$new_user_id) {
                    ...
                    if (1 == $this->getConfigValue('registration_notice')) {
                        $message = sprintf(Multilanguage::_('NEW_REGISTER_BODY', 'system'), $this->getRequestValue('login'), '***');
                        $subject = sprintf(Multilanguage::_('NEW_REGISTER_TITLE', 'system'), $_SERVER['HTTP_HOST']);
                    ...
                    $this->checkLogin($this->getRequestValue('login'), $this->getRequestValue('password'));
    ...
    function addUser($login, $password, $retype_password, $fio, $email, $captcha_string, $captcha_session_key)
    {
        ...
        if (!$this->checkIsLoginUsed($login)) {
     ...
     * @return boolean
     */
    function checkIsLoginUsed($login)
    {
    ...
    {
        return md5(time() . random_int(1000, 9999) . 'random key captcha string core sitebill');
    }
