...
<head>
    ...
    <script type="text/javascript">
        var options = {
            scrollZoom: {/literal}{if $scroll_zoom}true{else}false{/if}{literal},
            minimap: false,
            defaultZoom: 4,
            use_clusters: true
        };
    ...
    {elseif $map_type=='leaflet_osm'}
        ...
        <link rel="stylesheet" type="text/css"
              href="{$estate_folder}/apps/system/js/leaflet/MarkerCluster.Default.css"/>
    ...
    {else}
        <script type="text/javascript"
                src="https://maps.googleapis.com/maps/api/js?v=3{if $g_api_key ne ''}&key={$g_api_key}{/if}"></script>
        <script type="text/javascript"
                src="{$estate_folder}/apps/third/google/markerclusterer/markerclusterer.js"></script>
    {/if}
    ...
    <script src="{$estate_folder}/apps/system/js/activemap.js?v=5"></script>
<body style="margin:0;">
...
<div id="ActiveMapContainer" class="ActiveMapContainer" style="position: relative;">
                    ...
                    <div class="ActiveMapListBlock-items-item ActiveMapListBlock-tpl">
                        <div class="ActiveMapListBlock-item-root-do">
                            <a class="ActiveMapListBlock-item-link" target="_blank" href="">
                                <div class="ActiveMapListBlock-item-image"><img src="{$estate_folder}/img/no_foto.png">
                                </div>
    ...
    <div id="ActiveMap" class="" data-center-lat="{$map_center[0]}" data-center-lng="{$map_center[1]}"
         data-zoom="{$map_zoom}"{if isset($mapframe)} data-frame="{$mapframe}"{/if}></div>
...
{literal}
    <style>
        ...
        .fix-wide-map-search > section.advanced-search.advance-search-header {
            padding: 0 !important;
        }
        .fix-wide-map-search > section.advanced-search.advance-search-header > div > div > div > form > div.advance-fields {
            padding: 16px !important;
        }

        .fix-wide-map-search > section.advanced-search.advance-search-header > div > div > div > form > div.form-group.search-long > div.search-btn > input {
            border-radius: 0;
        }
        .fix-wide-map-search > section.advanced-search.advance-search-header > div > div > div > form > div.form-group.search-long > div.search {
            border-radius: 0;
        }
        .fix-wide-map-search > section.advanced-search.advance-search-header > div > div > div > form > div.form-group.search-long > div.search > div.row > div > button {
        ...
        .mobile-expand::-webkit-scrollbar {
            width: 12px;
        }

        .mobile-expand::-webkit-scrollbar-track {
            -webkit-box-shadow: inset 0 0 6px rgba(0,0,0,0.3);
            border-radius: 10px;
        }

        .mobile-expand::-webkit-scrollbar-thumb {
            border-radius: 10px;
        ...
        .advanced-search .bs-searchbox, .advanced-search-mobile .bs-searchbox {
            display: none !important;
        }
        #r_pricetype .bs-searchbox {
            display: none !important;
        }
