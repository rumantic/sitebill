...
define('IFRAME_MODE', true);
if ( is_array($_REQUEST['grid_params']) ) {
    $_SESSION['grid_params'] = $_REQUEST['grid_params'];
} else {
    $_REQUEST['grid_params'] = $_SESSION['grid_params'];
}

if ( is_array($_REQUEST['default_request']) ) {
    $_SESSION['default_request'] = $_REQUEST['default_request'];
} else {
    $_REQUEST['default_request'] = $_SESSION['default_request'];
}
if ( is_array($_REQUEST['default_request']) ) {
    foreach ( $_REQUEST['default_request'] as $key => $value ) {
        $_REQUEST[$key] = $value;
    }
}
