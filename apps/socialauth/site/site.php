...
class socialauth_site extends socialauth_admin {
    ...
    function frontend() {
        ...
        if ($REQUESTURIPATH == 'socialauth') {
            ...
            switch ($do) {
                case 'login_tg' : {
                        if ($this->getConfigValue('apps.socialauth.tg.enable')) {
                            require_once SITEBILL_DOCUMENT_ROOT . '/apps/socialauth/lib/tg/tg_logger.php';
                            $OL = Tg_Logger::getInstance();
                            $r = $OL->login();
                            if ($r) {
                            ...
                            if ($r) {
                                if ($_SESSION['user_id'] > 0) {
                                    if ($id = $this->checkExistingUser($_SESSION['ssAuthData']['ssType'], $_SESSION['ssAuthData']['id'])) {
                                        ...
                                        $DBC = DBC::getInstance();
                                        $query = 'UPDATE ' . DB_PREFIX . '_user SET `' . $_SESSION['ssAuthData']['ssType'] . '_id`=? WHERE user_id=?';
                                        $stmt = $DBC->query($query, array($_SESSION['ssAuthData']['id'], $_SESSION['user_id']));
                                        unset($_SESSION['ssAuthData']);
                                        header('location: ' . SITEBILL_MAIN_URL . '/account/profile');
                                        exit();
                                    }
                                } else {
                                    if ($id = $this->checkExistingUser($_SESSION['ssAuthData']['ssType'], $_SESSION['ssAuthData']['id'])) {
