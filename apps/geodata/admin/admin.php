...
class geodata_admin extends Object_Manager
{
    ...
    public function ajax()
    {
        if ($this->getRequestValue('action') == 'get_address_string') {
            ...
            $url = 'https://maps.googleapis.com/maps/api/place/autocomplete/json?input=' . $this->getRequestValue('input') . '&key=AIzaSyAE2c_0O3OqmJ7Gwn7fjhyLutG_rlLNuaA&components=country:ru&types[]=region';
            $curl = curl_init();
          ...
          echo $this->iframe_grid();
            ...
            $cache = $this->getGeocoderDataFromCache($str);
            //var_dump($cache);
            if (!empty($cache)) {
                ...
                if (!empty($result) && !isset($result['error'])) {
                    $r = $result;
                    unset($r['address']);
                    $this->setGeocoderDataToCache($str, $r);
                }
            }
        ...
        if (!empty($ids)) {
            foreach ($ids as $id) {
                ...
                if ($form_data_shared[$geofield]['value']['lat'] != '' && $form_data_shared[$geofield]['value']['lng'] != '') {
                    ...
                    $report[] = array(
                        'id' => $id,
                        'report' => 'Геокодинг невозможен. (' . $str . ' <span class="error">' . $this->getError() . '</span>)',
                        'error_status' => 1
                    );
    ...
    private function geocode($data)
    {
        ...
        $geodata = simplexml_load_file($str, $url);
        //var_dump($geodata);
    ...
    public static function geocode_address_by_google($address, $context = false)
    {
        $str = $address;
        if ($address == '') {
            if ( $context ) {
                $context->riseError('empty address');
            }
            return false;
        }
        ...
        if ( $context ) {
            $context->riseError('error on geocoding '.$output);
        }
        return false;
    }
                ...
                if ($str != '') {
                    ...
                    if (!empty($cache)) {
                        ...
                        if ('y' == $this->getConfigValue('apps.geodata.save_geocoder')) {
                            $res = $this->geocode_address($str, $this);

                            //echo $str.'<br/>';
                            //print_r($res);
                        } else {
                            $res = $this->geocode_address_by_google($str, $this);
                        }
                    }
    ...
    function geocode_address($address, $context = false)
    {
        $str = $address;
        if ($address == '') {
            if ( $context ) {
    ...
    {
        $DBC = DBC::getInstance();
        $query = 'INSERT INTO ' . DB_PREFIX . '_geodata_geocodercache (geocode_result, address_str) VALUES (?,?)';
        $stmt = $DBC->query($query, array(json_encode($g), $str));
    }

    public static function getGeocoderDataFromCache($str)
    {
        $DBC = DBC::getInstance();
        $query = 'SELECT geocode_result FROM ' . DB_PREFIX . '_geodata_geocodercache WHERE address_str=?';
        $stmt = $DBC->query($query, array(trim($str)));
        //file_put_contents(SITEBILL_DOCUMENT_ROOT.'/cache/geocache.req',$str."\r\n",FILE_APPEND);
        if ($stmt) {
            $ar = $DBC->fetch($stmt);
            $ar = json_decode($ar['geocode_result'], true);
            //var_dump($ar);
            return array('lat' => $ar['lat'], 'lng' => $ar['lng']);
        }
        return array();
    }
