...
class news_site extends news_admin {
	...
	public function front_news_grid($topic_id=0){
		...
		if($topic_id!=0){
			$breadcrumbs[]='<a href="'.(SITEBILL_MAIN_URL!='' ? SITEBILL_MAIN_URL : '').'/'.$app_alias.$trailing_slashe.'">'.$app_title.'</a>';
		...
		if(1==$this->getConfigValue('apps.news.use_news_topics')){
			...
			if($stmt){
				...
				if($url!=$ar['url']){
					...
					if(false!==strpos($ar['url'], '.')){
						$trailing_slashe='';
					}
					$new_location=SITEBILL_MAIN_URL.'/'.$app_alias.'/'.$ar['url'].$trailing_slashe;
					$this->go301($new_location);
				}
				return $ar['id'];
			}
	...
	function frontend () {
    	...
    	return false;	
    		...
    		/*if(preg_match('/^'.$app_item_alias.'(\d+).html$/', $REQUESTURIPATH, $matches)){
				...
				if(isset($news['newsalias']) && $news['newsalias']['value']!=''){
					$href=$this->getNewsRoute($news['id']['value'], $news['newsalias']['value']);
					$this->go301($href);
				}
    			...
    			$this->template->assert('meta_description', $this->getConfigValue('apps.news.meta_desription'));
    			$this->template->assert('meta_keywords', $this->getConfigValue('apps.news.meta_keywords'));
	            ...
	            if ( $this->getConfigValue('apps.news.folder_title') != '' ) {
	            	$breadcrumbs[]='<a href="'.(SITEBILL_MAIN_URL!='' ? SITEBILL_MAIN_URL : '').'/'.$app_alias.$trailing_slashe.'">'.$this->getConfigValue('apps.news.folder_title').'</a>';
	            } else {
	            	$breadcrumbs[]='<a href="'.(SITEBILL_MAIN_URL!='' ? SITEBILL_MAIN_URL : '').'/'.$app_alias.$trailing_slashe.'">'.Multilanguage::_('PAGE_TITLE','news').'</a>';
	            }
				...
				if(isset($news['meta_title']['value']) && $news['meta_title']['value']!=''){
					...
					$this->template->assert('meta_title', $news['title']['value']);
				}
				if(isset($news['meta_description']['value']) && $news['meta_description']['value']!=''){
				...
				$this->set_apps_template('news', $this->getConfigValue('theme'), 'main_file_tpl', 'news_view.tpl');
    			return true;
    		}elseif(preg_match('/'.$app_alias.'\/(.*)[\/]?/', $REQUESTURIPATH, $matches)){
