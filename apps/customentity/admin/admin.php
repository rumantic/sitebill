...
class customentity_admin extends Object_Manager {
    ...
    public static function reset_custom_entity_holder () {
        self::$custom_entity_holder = false;
    }

    public static function getEntityList($admin_prefix = 'admin'){
    	...
    	if(false!==self::$custom_entity_holder){
    		...
    		if($stmt){
    			while($ar=$DBC->fetch($stmt)){
                    ...
                    $ret[$ar['entity_name']]['title']=$ar['entity_title'];
   					$ret[$ar['entity_name']]['href']=SITEBILL_MAIN_URL.'/'.$admin_prefix.'?action='.$ar['entity_name'];
    			}
    		}
    		if ( !empty($ret) ) {
