 ...
 */
class Onelevel_Menu extends Structure_Manager {
        ...
        foreach ($category_structure['childs'][0] as $item_id => $categoryID) {
            ...
            if ($category_structure['catalog'][$categoryID]['url'] != '') {
                if (preg_match('/^http/', $category_structure['catalog'][$categoryID]['url'])) {
                    $rs .= '<li class=" ' . ($category_structure['catalog'][$categoryID]['current'] == 1 ? ' current' : '') . '"><a class="pureCssMenui" href="' . $category_structure['catalog'][$categoryID]['url'] . '">' . $name . '</a>';
                } else {
                    $rs .= '<li class="' . ($category_structure['catalog'][$categoryID]['current'] == 1 ? ' current' : '') . '" ><a class="pureCssMenui" href="' . SITEBILL_MAIN_URL . '/' . $category_structure['catalog'][$categoryID]['url'] . '">' . $name . '</a>';
                }
            } else {
                $rs .= '<li class="' . ($category_structure['catalog'][$categoryID]['current'] == 1 ? ' current' : '') . '"><a class="pureCssMenui" href="' . SITEBILL_MAIN_URL . '/topic' . $categoryID . '.html">' . $name . '</a>';
            }
