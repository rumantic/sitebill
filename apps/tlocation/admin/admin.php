...
class tlocation_admin extends Object_Manager {
            ...
            default : {
                    ...
                    foreach ($data_model_shared as $key => $value) {
                        if ($value['type'] == 'select_by_query' && $value['primary_key_table'] == $from) {
                            //print_r($value);
                            return $this->_getAbstractData($term, $value['primary_key_table'], $value['primary_key_name'], $value['value_name'], $dep_key, $dep_val);
                        }

                        // @todo: Сильно не пинайте, но эта кастомизация нужна для экспериментального проекта.
                        if ($value['name'] == 'contact_id' && $value['primary_key_table'] == $from) {
                            //print_r($value);
                            return $this->_getAbstractData($term, $value['primary_key_table'], $value['primary_key_name'], $value['value_name'], $dep_key, $dep_val, 'contact_decorator');
                        }
    ...
    private function _getAbstractData($term, $table, $key, $field, $dep_key = '', $dep_val = 0, $decorator_function = null) {
        $ret = array();
        $DBC = DBC::getInstance();
        $autocomplete_distinct = false;
        if(1 == $this->getConfigValue('autocomplete_distinct')){
            $autocomplete_distinct = true;
        }
        
        $searchable_term = '%'.$term.'%';
        
        if($dep_key!='' && $dep_val!=0){
            $query = 'SELECT ' . $key . ', ' . $field . ' FROM ' . DB_PREFIX . '_' . $table . ' WHERE ' . $field . ' LIKE ? AND `'.$dep_key.'`=?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY ' . $field . ' ASC LIMIT 50';
            $stmt = $DBC->query($query, array($searchable_term, $dep_val));
        }else{
            $query = 'SELECT ' . $key . ', ' . $field . ' FROM ' . DB_PREFIX . '_' . $table . ' WHERE ' . $field . ' LIKE ?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY ' . $field . ' ASC LIMIT 50';
        ...
        if ($stmt) {
            while ($ar = $DBC->fetch($stmt)) {
                ...
                $field_value = SiteBill::iconv(SITE_ENCODING, 'utf-8', $ar[$field]);
                if ( !is_null($decorator_function) and method_exists($this, $decorator_function) ) {
                    $field_value .= ' '.$this->$decorator_function($ar[$key]);
                }
                $ret[] = array($key => $ar[$key], $field => $field_value, 'pos' => $pos, 'field' => $field);
            }
        }
    ...
    private function contact_decorator ( $contact_id ) {
        $DBC = DBC::getInstance();
        $query = "SELECT * from ".DB_PREFIX."_client cli, ".DB_PREFIX."_contact cont  where cli.id=cont.client_id and cont.id=?";
        $stmt = $DBC->query($query, array($contact_id));
        if ( $stmt ) {
            $ar = $DBC->fetch($stmt);
            return $ar['name'];
        }
    ...
    private function _getStreets($term, $dep_key = '', $dep_val = 0) {
        ...
        if(1===intval($this->getConfigValue('apps.language.use_langs'))){
            ...
            $lang_key = 'name';
            if (isset($data_model_shared['data']['street_id']) && 0 == intval($data_model_shared['data']['street_id']['parameters']['no_ml'])) {
                if ($lang != 'ru') {
                    $lang_key = 'name_' . $lang;
                } else {
                    $lang_key = 'name';
        ...
        if($dep_key!='' && $dep_val!=0){
            $query='SELECT street_id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_street WHERE `'.$lang_key.'` LIKE ? AND `'.$dep_key.'`=?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY `'.$lang_key.'` ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term, $dep_val));
        }else{
            $query='SELECT street_id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_street WHERE `'.$lang_key.'` LIKE ?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY name ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term));
        }
    ...
    private function _getDistricts($term, $dep_key = '', $dep_val = 0) {
        ...
        if (1 === intval($this->getConfigValue('apps.language.use_langs'))) {
            ...
            $data_model_shared = $data_model->get_kvartira_model(false, true);
            $lang_key = 'name';
            if (isset($data_model_shared['data']['district_id']) && 0 == intval($data_model_shared['data']['district_id']['parameters']['no_ml'])) {
                if ($lang != 'ru') {
                    $lang_key = 'name_' . $lang;
                } else {
                    $lang_key = 'name';
                }
            } elseif (isset($data_model_shared['data']['district_id']) && 1 == intval($data_model_shared['data']['district_id']['parameters']['no_ml'])) {
                $lang_key = 'name';
            }
        } else {
            $lang_key = 'name';
        ...
        if($dep_key!='' && $dep_val!=0){
            $query='SELECT id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_district WHERE `'.$lang_key.'` LIKE ? AND `'.$dep_key.'`=?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY `'.$lang_key.'` ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term, $dep_val));
        }else{
            $query='SELECT id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_district WHERE `'.$lang_key.'` LIKE ?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY name ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term));
        }
    ...
    private function _getCities($term, $dep_key = '', $dep_val = 0) {
        ...
        if(1===intval($this->getConfigValue('apps.language.use_langs'))){
            ...
            $data_model_shared = $data_model->get_kvartira_model(false, true);
            $lang_key = 'name';
            if(isset($data_model_shared['data']['city_id']) && 0==intval($data_model_shared['data']['city_id']['parameters']['no_ml'])){
                if($lang!='ru'){
                    $lang_key = 'name_'.$lang;
                }else{
                    $lang_key = 'name';
                }
            }elseif(isset($data_model_shared['data']['city_id']) && 1==intval($data_model_shared['data']['city_id']['parameters']['no_ml'])){
                $lang_key = 'name';
            }
        }else{
            $lang_key = 'name';
        ...
        if($dep_key!='' && $dep_val!=0){
            $query='SELECT city_id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_city WHERE `'.$lang_key.'` LIKE ? AND `'.$dep_key.'`=?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY `'.$lang_key.'` ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term, $dep_val));
        }else{
            $query='SELECT city_id, `'.$lang_key.'` AS name FROM '.DB_PREFIX.'_city WHERE `'.$lang_key.'` LIKE ?'.($autocomplete_distinct ? ' GROUP BY name' : '').' ORDER BY name ASC LIMIT 50';
            $stmt=$DBC->query($query, array($searchable_term));
        }
