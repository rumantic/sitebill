...
class toolbox_admin extends Object_Manager {
    ...
    protected function _defaultAction() {
        ...
        $ret .= '<a class="btn" href="' . SITEBILL_MAIN_URL . '/admin/index.php?action=' . $this->action . '&do=render_watermark_again" onclick="return confirm(\'Данная операция перезапишет водяной знак поверх ваших изображений. Продолжить?\');">Перезаписать водяные знаки для больших и превью изображений</a> ';
        ...
        //$ret .= '<a class="btn" href="' . SITEBILL_MAIN_URL . '/admin/index.php?action=' . $this->action . '&do=csrfSalt">Генерирование CSRF-соли</a> ';
        
        $ua = array();
        $d = SITEBILL_DOCUMENT_ROOT.'/apps/toolbox/useractions';
        if (is_dir($d)) {
    		if ($dh = opendir($d)) {
    			while (($action = readdir($dh)) !== false) {
                    if (is_file($d . '/' . $action) && !preg_match('/^\./', $action)) {
                        $action_name = preg_replace('/\.php$/', '', $action);
                        require_once $d . '/' . $action;
                        $obj = new $action_name();
                        $ua[$action_name] = $obj->getName();
                    }
    			}
    			closedir($dh);
    		}
        ...
        if(!empty($ua)){
            $ret .= '<h1>Пользовательские</h1> ';
            foreach($ua as $code=>$name){
                $ret .= '<a class="btn" href="' . SITEBILL_MAIN_URL . '/admin/index.php?action=' . $this->action . '&do=runuseraction&code='.$code.'">'.$name.'</a> ';
            }
        }
    ...
    protected function _runuseractionAction() {
        $ret = '';
        $code = trim($_GET['code']);
        ...
        if(file_exists($d.'/'.$code.'.php')){
            require_once $d.'/'.$code.'.php';
            $obj = new $code();
            $ret = $ua[$action_name] = $obj->run();
        }
    ...
    /*protected function _csrfSaltAction(){
        $DBC = DBC::getInstance();
        $query = 'UPDATE '.DB_PREFIX.'_config SET `value`=? WHERE `config_key`=?';
        $DBC->query($query, array($this->generateCSRFToken(), 'csrf_salt'));
    ...
    protected function _init_metaphoneAction() {
        //$rs .= $this->update_table_metaphone('city', 'city_id', 'name');
        //$rs .= $this->update_table_metaphone('street', 'street_id', 'name');
        $rs .= $this->update_table_metaphone('community', 'community_id', 'name');
        //$rs .= $this->update_table_metaphone('street', 'street_id', 'name');
        return $rs;
    }

    protected function update_table_metaphone($table_name, $primary_key, $name_column) {
        $rs = 'Генерация метафона для ' . $table_name . ', primary_key = ' . $primary_key . ', column = ' . $name_column . '<br>';
        $this->db->exec("ALTER TABLE " . DB_PREFIX . "_" . $table_name . " ADD COLUMN metaphone VARCHAR(60) DEFAULT NULL");
        $rs .= $this->db->error . '<br>';
        $this->db->exec("SELECT " . $primary_key . ", " . $name_column . " FROM " . DB_PREFIX . "_" . $table_name);
        $rs .= $this->db->error . '<br>';
        while ($row = $this->db->fetch_assoc()) {
            // находим метафон и записываем его в таблицу
            $metaphone[$row[$primary_key]]['metaphone'] = $this->mtphn($row[$name_column]);
            $metaphone[$row[$primary_key]][$name_column] = $row[$name_column];
        }
        foreach ($metaphone as $key_id => $key_info) {
            $query = "update " . DB_PREFIX . "_" . $table_name . " set metaphone='" . $key_info['metaphone'] . "' where $primary_key=$key_id";
            //echo $query . '<br>';
            $this->db->exec($query);
            if ($this->db->success) {
                $rs .= $key_info[$name_column] . ' = ' . $key_info['metaphone'] . '<br>';
            } else {
                $rs .= $this->db->error . '<br>';
            }
        }
    ...
    protected function _render_watermark_againAction() {
        $start = (int) $this->getRequestValue('start');
        $step = 10;
                    ...
                    if (!empty($images)) {
                        foreach ($images as $img) {
                            ...
                            if (file_exists(SITEBILL_DOCUMENT_ROOT . '/img/data/' . $prev)) {
                                $Watermark->printWatermark(SITEBILL_DOCUMENT_ROOT . '/img/data/' . $prev, true);

                                $rs .= 'image ' . SITEBILL_DOCUMENT_ROOT . '/img/data/' . $prev . ' [rendered]<br>';
                            }

                            if (file_exists(SITEBILL_DOCUMENT_ROOT . '/img/data/' . $normal)) {
                                $Watermark->printWatermark(SITEBILL_DOCUMENT_ROOT . '/img/data/' . $normal);
                                $rs .= 'image ' . SITEBILL_DOCUMENT_ROOT . '/img/data/' . $normal . ' [rendered]<br>';
                            }
                        }
                    }
                }
        ...
        $rs .= '<br> Выполнить следующий шаг <a href="' . SITEBILL_MAIN_URL . '/admin/index.php?action=' . $this->action . '&do=render_watermark_again&start=' . ($start + $step) . '">далее</a>';
        return $rs;
    ...
    function _fromUploadifyProductAction() {
        $rs = '';
        if (isset($_POST['uploads_field']) && trim($_POST['uploads_field']) != '') {
            $rs .= 'Запуск миграции картинок для поля ' . $_POST['uploads_field'] . '<br>';
            $field = trim($_POST['uploads_field']);
            ...
            $pdata = array();
            $query = 'SELECT product_id, `' . $field . '` FROM ' . DB_PREFIX . '_product WHERE `' . $field . '`<>\'\'';
            $stmt = $DBC->query($query);
            if ($stmt) {
                while ($ar = $DBC->fetch($stmt)) {
                    $x = unserialize($ar[$field]);
            ...
            if (!empty($pdata)) {
                ...
                $query = 'UPDATE ' . DB_PREFIX . '_product SET `' . $field . '`=\'\' WHERE product_id=?';
                foreach ($pdata as $id) {
                    $stmt = $DBC->query($query, array($id));
                }
            } else {
            ...
            $query = 'SELECT di.product_id, i.* FROM ' . DB_PREFIX . '_product_image di LEFT JOIN ' . DB_PREFIX . '_image i USING(image_id) ORDER BY di.sort_order';
            //echo $query.'<br>';
            $stmt = $DBC->query($query);
            if ($stmt) {
                while ($ar = $DBC->fetch($stmt)) {
            ...
            if (!empty($images)) {
                ...
                $query = 'UPDATE ' . DB_PREFIX . '_product SET `' . $field . '`=? WHERE `' . $field . '`=\'\' AND product_id=?';
                foreach ($images as $id => $idimages) {
                    $stmt = $DBC->query($query, array(serialize($idimages), $id));
                }
            }
            ...
            return '<form action="' . SITEBILL_MAIN_URL . '/admin/index.php?action=' . $this->action . '&do=fromUploadifyProduct" method="post">Системное имя поля Uploads для картинок <input type="text" name="uploads_field"> <input type="submit" name="submit" value="Пуск" class="btn"></form>';
        }
    }
