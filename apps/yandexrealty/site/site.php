...
class yandexrealty_site extends yandexrealty_admin
{
    ...
    public function run_export()
    {
        ...
        foreach ($data as $data_item) {
            ...
            $xml_collectorp = array_merge($xml_collectorp, $xml_collector_item, $this->commonExporter->compileRules($data_full_model[$data_item['id']]));
    ...
    protected function collectItemXML($data_item, $hasUploadify, $uploadsField)
    {
        ...
        $xml_collectorp[] = $this->exIsImageOrderChangeAllowed($data_item);
    ...
    protected function exIsImageOrderChangeAllowed($data_item)
    {
        return '<is-image-order-change-allowed>0</is-image-order-change-allowed>';
    }
    ...
    protected function collectData($only_ids = false)
    {
        ...
        if (!empty($tasks)) {
            ...
            if (!is_null($this->exported_ids)) {
                ...
                if (
                    '' !== trim($this->getConfigValue('apps.yandexrealty.filtering_checkbox_name'))
                    and
                    !$force_mode
                ) {
                    $where[] = 'dt.' . trim($this->getConfigValue('apps.yandexrealty.filtering_checkbox_name')) . '=1';
    ...
    protected function get_ids_array () {
        return $this->ids_array;
    }

    protected function getWhereConditions($where)
    {
        foreach ($this->form_data_shared as $key => $item) {
            if ($this->request()->get($key) == 1 and $item['type'] == \system\types\model\Dictionary::CHECKBOX) {
                $where[] = 'dt.' . $key . '=1';
            }
        }
        return $where;
