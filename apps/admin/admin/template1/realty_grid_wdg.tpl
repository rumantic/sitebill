...
<div id="grid_control_panel">С выделенными 
    			<select name="cp_optype">
    			<option>выбрать</option>
    			<option value="topic_id">переместить в категорию</option>
    			<option value="active">установить активность</option>
    			<option value="hot">спецразмещение</option>
    			</select>
    			<div id="grid_control_panel_content"></div>
    			<button alt="data" id="run" style="display:none;" class="btn btn-primary">Выполнить</button>
    			</div>
...
{/section}
<tr>
		<td colspan="13"><button alt="data" class="delete_checked btn btn-danger"><i class="icon-white icon-remove"></i> {$L_DELETE_CHECKED}</button></td>
	</tr>
	{if $pager != ''}
	<tr>
		<td colspan="13" class="pager"><div align="center">{$pager}</div></td>
	</tr>
	{/if}
</table>