            ...
            try {
                tag_input.tag({
                    ...
                    source: function (query, process) {
                        ...
                        $.ajax({url: estate_folder + '/js/ajax.php?action=get_tags&term=' + query + '&column_name=' + column_name + '&model_name=data'}).done(function (result_items) {
...
<div class="navbar">
    <div class="navbar-inner">
        <div class="container">
            <a class="brand" href="#">{_e t="Найдено"}: {$_total_records}</a>
            <div class="nav pull-right">

                {if $admin ne ''}
                <div align="right"><a href="?action=data&do=import" title="Загрузить записи в формате Excel"
                                      class="btn btn-info btn-xs "><i class="icon-white icon-upload"></i> </a> <a
                            href="#search" id="search_toggle" class="btn btn-info"><i
                                class="icon-white icon-search"></i> {$L_ADVSEARCH}</a></div>
                <div id="search_form_block"
                     {if $smarty.request.submit_search_form_block eq ''}style="display:none;"{/if} class="spacer-top">
					...
					<div class="control-group">
					  ...
					  -->
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <input type="text" name="srch_date_from" id="srch_date_from"
                                       value="{$smarty.request.srch_date_from}"/>
                            </div>
                        </div>
                        <div class="control-group">
                            <label class="control-label">{$L_DATE} {$L_TO}</label>
                            <div class="controls">
                                <input type="text" name="srch_date_to" id="srch_date_to"
                                       value="{$smarty.request.srch_date_to}"/>
                            </div>
                        </div>
                        ...
                        <div class="control-group">
                            <div class="controls">
                                <input type="submit" name="submit_search_form_block" value="{$L_GO_FIND}"
...
<table class="table table-hover standart_admin_grid w-full" cellspacing="2" cellpadding="2">
    <tr class="row_head">
        <td width="1%" class="row_title"><input type="checkbox" id="grid_check_all" class="grid_check_all ace"/><label
                    for="grid_check_all" class="lbl"></label></td>
        ...
        {if $show_uniq_id}
            <td width="1%" class="row_title">
                <!-- #section:plugins/input.tag-input -->
                <div class="inline-tags">
                    <input type="text" name="uniq_id" id="uniq_id" class="input-tag tagged safe_string" value=""
                           placeholder="..."/>
                </div>
        ...
        <td width="1%" class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="date_added" id="date_added" class="input-tag tagged dtdatetime" value=""
                       placeholder="..."/>
            </div>
        ...
        <td width="70" class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="topic_id" id="topic_id" class="input-tag tagged structure" value=""
                       placeholder="..."/>
            </div>
        ...
        <td width=13% class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="city_id" id="city_id" class="input-tag tagged select_by_query" value=""
                       placeholder="..."/>
            </div>
        ...
        <td width=13% class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="district_id" id="district_id" class="input-tag tagged select_by_query" value=""
                       placeholder="..."/>
            </div>
            ...
            {/if}
            <!-- /section:plugins/input.tag-input -->
            {$L_DISTRICT}&nbsp;<a href="{$url}&order=district&asc=asc">&darr;</a>&nbsp;<a
                    href="{$url}&order=district&asc=desc">&uarr;</a>
        </td>
        <td width=13% class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="street_id" id="street_id" class="input-tag tagged select_by_query" value=""
                       placeholder="..."/>
            </div>
            ...
            {/if}
            <!-- /section:plugins/input.tag-input -->
            {$L_STREET}&nbsp;<a href="{$url}&order=street&asc=asc">&darr;</a>&nbsp;<a
                    href="{$url}&order=street&asc=desc">&uarr;</a>
        </td>
        <td width=13% class="row_title">
            <!-- #section:plugins/input.tag-input -->
            <div class="inline-tags">
                <input type="text" name="number" id="number" class="input-tag tagged safe_string" value=""
                       placeholder="..."/>
            </div>
            ...
            <div class="inline-tags">
                <div class="ranged-tags" data-field="price">
                    ...
                    <div class="ranged-tags-params" style="display: none;">
                        <input name="price[min]" type="text" class="tagged_input"
                               value="{$smarty.session.tags_array.price.min}">
                        <input name="price[max]" type="text" class="tagged_input"
                               value="{$smarty.session.tags_array.price.max}">
        ...
        {else}
            <td class="row_title">
                <!-- #section:plugins/input.tag-input -->
                <div class="inline-tags">
                    <input type="text" name="user_id" id="user_id" class="input-tag tagged select_by_query" value=""
                           placeholder="..."/>
                </div>
    ...
    {section name=i loop=$grid_items}
        <tr valign="top"
            class="item {if $grid_items[i].hot}row3hot{else}row3{/if}{if $grid_items[i].active == 0} notactive{/if}">
            <td>
                <input id="grid_check_all_{$grid_items[i].id}" type="checkbox" class="grid_check_one ace"
                       value="{$grid_items[i].id}"/><label for="grid_check_all_{$grid_items[i].id}" class="lbl"></label>
            ...
            <td>
                {if $grid_items[i].city_id!=0}
                    <a href="{$estate_folder}/admin/?action=data&city_id={$grid_items[i].city_id}" data-rel="tooltip"
                       title="Показать все объявления для {$grid_items[i].city}"><i class="icon-white icon-filter"></i></a>
                {/if}
                {$grid_items[i].city}
            </td>
            <td>
			...
			{if $grid_items[i].district_id!=0}
                <a href="{$estate_folder}/admin/?action=data&district_id={$grid_items[i].district_id}"
                   data-rel="tooltip" title="Показать все объявления для {$grid_items[i].district}"><i
                            class="icon-white icon-filter"></i></a>
            {/if}
			...
			{if $grid_items[i].street_id!=0}
                <a href="{$estate_folder}/admin/?action=data&street_id={$grid_items[i].street_id}" data-rel="tooltip"
                   title="Показать все объявления для {$grid_items[i].street}"><i
                            class="icon-white icon-filter"></i></a>
            {/if}
            ...
            <td>
                ...
                {else}
                    <a href="{$estate_folder}/admin/?action=data&user_id={$grid_items[i].user_id}" data-rel="tooltip"
                       title="Показать все объявления для {$grid_items[i].user}"><i class="icon-white icon-filter"></i></a>
                    <span class="user-info2 userinfo_tooltipe_block" data-rel="popover" data-html="true"
                          data-placement="left" data-user-id="{$grid_items[i].user_id}">{$grid_items[i].user}
                        {if isset($grid_items[i]._user_info) && isset($grid_items[i]._user_info.login)}
                            <br/>
                            ({$grid_items[i]._user_info.login}){/if}</span>
                {/if}
            </td>
            {if $admin !=''}
                <td nowrap>
                    ...
                    {else}
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu">
                                <li><a href="#" alt="{$grid_items[i].id}" title="Активация"
                                       class="btn {if $grid_items[i].active == 0}item-on btn-danger{else}item-off btn-success{/if}"><i
                                                class="icon-white icon-off"></i> Активность</a></li>
                                {if $grid_items[i].active == 1}
                                    <li><a title="На сайте" href="{$grid_items[i].href}" target="_blank"
                                           class="btn btn-success"><i class="icon-white icon-forward"></i> На сайте</a>
                                    </li>
                                {/if}
                                {if isset($show_contacts_enable) && $show_contacts_enable}

                                    {if $grid_items[i].show_contact eq 0}
                                        <li><img src="{$estate_folder}/img/contact_delete_16x16.gif"
                                                 alt="{$L_CONTACTS_ARE_HIDE}" title="{$L_CONTACTS_ARE_HIDE}" border="0"
                                                 width="16" height="16"/></li>
                                    {else}
                                        <li><img src="{$estate_folder}/img/contact-new.png"
                                                 alt="{$L_CONTACTS_ARE_SHOWED}" title="{$L_CONTACTS_ARE_SHOWED}"
                                ...
                                {if isset($sms_enable) && $sms_enable}
                                    <li><a class="btn btn-success"
                                           onclick="return confirm({literal}'{$L_MESSAGE_REALLY_WANT_SMS}'{/literal});"
                                           href="{$estate_folder_control}?do=structure&subdo=sms&id={$grid_items[i].id}"><img
                                                    src="{$estate_folder}/img/sms16x16.png" alt="{$L_SENDSMS_LC}"
                                                    title="{$L_SENDSMS_LC}" border="0" width="16" height="16"/> SMS</a>
                                    </li>
                                {/if}
                                {if isset($show_up_icon) && $show_up_icon}
                                    <a class="btn btn-warning go_up" alt="{$grid_items[i].id}" href="#grow_up"><i
                                                class="icon-white icon-circle-arrow-up"></i></a>
                                {/if}
                            </ul>
                        </div>
                        <div class="btn-group">
                            <a title="Редактировать" href="{$estate_folder_control}?do=edit&id={$grid_items[i].id}"
                               class="btn btn-info"><i class="icon-white icon-pencil"></i></a>
                            {if intval($grid_items[i].archived) === 1}
                                <a onclick="return confirm('{$L_MESSAGE_REALLY_WANT_DELETE}');"
                                   href="{$estate_folder_control}?action=data&do=delete_final&id={$grid_items[i].id}"
                                   class="btn btn-danger"><i class="icon-white icon-remove"></i></a>
                            {else}
                                <a onclick="return confirm('{$L_MESSAGE_REALLY_WANT_DELETE}');"
                                   href="{$estate_folder_control}?action=data&{if $topic_id != ''}topic_id={$topic_id}&{/if}do=delete&id={$grid_items[i].id}"
                                   class="btn btn-danger"><i class="icon-white icon-remove"></i></a>
                            {/if}
                        </div>
                    {/if}
    ...
    <tr>
        <td colspan="14" class="!p-3">
            <div class="flex flex-wrap items-center gap-2">
                <button alt="data" class="delete_checked inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-[13px] font-medium text-rose-600 bg-rose-50 hover:bg-rose-100 transition-colors"><i
                            class="icon-remove"></i> {$L_DELETE_CHECKED}</button>
                <button alt="data" class="batch_update inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-[13px] font-medium text-slate-600 border border-slate-200 hover:bg-slate-50 transition-colors"><i class="icon-th"></i> Пакетная
                    обработка
                </button>
                <button alt="data" class="duplicate inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-[13px] font-medium text-slate-600 border border-slate-200 hover:bg-slate-50 transition-colors"><i class="icon-th"></i> Дублировать</button>
            </div>
        </td>
    </tr>
    {if $pager != ''}
