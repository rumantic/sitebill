...
*/
            ...
            $('.tags-clear').click(function(e){
                e.preventDefault();
                SitebillCore.lajax({
                    url: estate_folder + '/js/ajax.php?action=get_tags&do=clear'
                }).done(function () {
                    location.reload();
                });
            });
                ...
                try {
                    tag_input.tag({
                        ...
                        source: function (query, process) {
                            ...
                            SitebillCore.lajax({
                                url: estate_folder + '/js/ajax.php?action=get_tags&column_name=' + column_name + '&model_name=data&query=' + query + '&term=' + query
            ...
            $('.checkboxfield').each(function (e) {
                const _this = $(this);
                const name = _this.data('field');
                if (typeof column_values_for_tags[name] != 'undefined' && column_values_for_tags[name].length > 0) {
                    tag_array = [];
                    for (let i in column_values_for_tags[name]) {
                        tag_array.push(column_values_for_tags[name][i]);
                        datastr[name] = tag_array;
                    }
                }
...
<table class="table table-bordered dataTable new_admin_grid" >
    <thead>
        <tr>
            <th rowspan="2"><input type="checkbox" class="grid_check_all ace" /><label for="grid_check_all" class="lbl"></label></th>
            {if $admin_grid_leftbuttons==1}
                <th rowspan="2"><a class="tags-clear" href="">{_word key='admin.CLEAR_ALL_FILTERS'}</a></th>
            {/if}
            ...
            {foreach from=$grid_data_columns item=grid_data_column}
                <th>
                    {if !in_array($core_model[$grid_data_column].type, array('uploads', 'docuploads', 'uploadify_image'))}
                        ...
                        {elseif $core_model[$grid_data_column].type == 'checkbox'}
                            <select class="checkboxfield" data-field="{$grid_data_column}">
                                <option value="">{_word key='admin.CHECKBOX_FIELD_FILTER_ANY'}</option>
                                <option value="1"{if isset($smarty.session.tags_array[$grid_data_column]) && in_array('1', $smarty.session.tags_array[$grid_data_column])} selected="selected"{/if}>{_word key='admin.CHECKBOX_FIELD_FILTER_CHECKED'}</option>
                                <option value="0"{if isset($smarty.session.tags_array[$grid_data_column]) && in_array('0', $smarty.session.tags_array[$grid_data_column])} selected="selected"{/if}>{_word key='admin.CHECKBOX_FIELD_FILTER_UNCHECKED'}</option>
                            </select>
                        {else}
                            <div class="inline-tags">
    ...
    {section name=i loop=$grid_items}
        <tr valign="top"
            ...
            {foreach from=$grid_data_columns item=grid_data_column}
                ...
                {elseif $grid_items[i][$grid_data_column].type=='uploads'}
                    <td>
                        {if is_array($grid_items[i][$grid_data_column].value) && !empty($grid_items[i][$grid_data_column].value)}
                            <div class="colorboxed" style="max-width: 150px;" data-cbxid="{$grid_items[i].id.value}">
                                <div class="photoslider{$grid_items[i].id.value}">
                                    {foreach from=$grid_items[i][$grid_data_column].value item=image key=k}
                                        <a {if $k != 0}style="display: none;"{/if} href="{mediaincpath data=$image}" data-thumb="{mediaincpath data=$image type='preview'}"></a>
                                    {/foreach}
                                </div>
                            </div>
                        {/if}
                ...
                {elseif $grid_items[i][$grid_data_column].type=='select_by_query'}
                    <td>{$grid_items[i][$grid_data_column].value_string} <small>({$grid_items[i][$grid_data_column].value})</small></td>
                {else}
                    <td>
