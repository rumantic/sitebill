            ...
            <div class="nav pull-right">
                ...
                <div id="search_form_block"
                    ...
                    <table>
                        <tr>
                            <td>{$L_WORD}</td>
                            <td><input type="text" name="srch_word" value="{$smarty.request.srch_word}"/></td>
                        </tr>
                        <tr>
                            <td>{$L_PHONE}</td>
                            <td><input type="text" name="srch_phone" value="{$smarty.request.srch_phone}"/></td>
                        </tr>
                        <tr>
                            <td>{$L_ID}</td>
                            <td><input type="text" name="srch_id" value="{$smarty.request.srch_id}"/></td>
                        </tr>
                        {if $show_uniq_id}
                            <tr>
                                <td>UNIQ_ID</td>
                                <td><input type="text" name="uniq_id" value="{$smarty.request.uniq_id}"/></td>
                            </tr>
                        {/if}
                        <tr>
                            <td>{$L_DATE} {$L_FROM}</td>
                            <td><input type="text" name="srch_date_from" id="srch_date_from"
                                       value="{$smarty.request.srch_date_from}"/></td>
                        </tr>
                        <tr>
                            <td>{$L_DATE} {$L_TO}</td>
                            <td><input type="text" name="srch_date_to" id="srch_date_to"
                                       value="{$smarty.request.srch_date_to}"/></td>
                        </tr>
                        <tr>
                            <td></td>
                            <td align="right">
                                ...
                                <input type="button" id="reset" value="{$L_RESET}" class="btn btn-warning"/></td>
                        </tr>
                    </table>
                    </form>
                </div>
...
<table class="table table-bordered dataTable new_admin_grid">
    ...
    <tr>
        ...
        {foreach from=$grid_data_columns item=grid_data_column}
            <th {if $smarty.request.order eq $grid_data_column}class="sorting_{if $smarty.request.asc eq 'desc'}desc{else}asc{/if}"
                {else}class="sorting"{/if} >
                <!-- #section:plugins/input.tag-input -->
                <div class="inline-tags">
                    <input type="text" name="{$grid_data_column}" id="{$grid_data_column}" class="input-tag" value=""
                           placeholder="..."/>
                </div>
    ...
    {section name=i loop=$grid_items}
        <tr valign="top"
            class="{if $grid_items[i].hot.value}row3hot{/if}{if intval($grid_items[i].status_id.value)>0} row_status_id{$grid_items[i].status_id.value}{/if}{if $grid_items[i].active.value == 0} notactive{/if}">

            <td><input type="checkbox" class="grid_check_one" value="{$grid_items[i].id.value}"/></td>
            <td>
                <button alt="data" class="fast_preview btn btn-danger"><i class="icon-white icon-eye-open"></i></button>
            </td>
            {foreach from=$grid_data_columns item=grid_data_column}
                        ...
                        <ul class="ace-thumbnails clearfix">
                            <li>
                                <a href="{$estate_folder}/img/data/{$grid_items[i][$grid_data_column].value[0].normal}"
                                   data-rel="colorbox{$grid_items[i].id.value}">
                                    <img src="{$estate_folder}/img/data/{$grid_items[i][$grid_data_column].value[0].preview}"
                            ...
                            {foreach from=$grid_items[i][$grid_data_column].value item=image key=k}
                                {if $k != 0}
                                    <li style="display: none;">
                                        <a href="{$estate_folder}/img/data/{$image.normal}"
                                           data-rel="colorbox{$grid_items[i].id.value}"><img
                                                    src="{$estate_folder}/img/data/{$image.preview}" width="50"/></a>
                                    </li>
                                {/if}
                            {/foreach}
                        </ul>
                ...
                {elseif $grid_items[i][$grid_data_column].type=='geodata' && is_array($grid_items[i][$grid_data_column].value)}
                    <td>{$grid_items[i][$grid_data_column].value_string.lat}
                        , {$grid_items[i][$grid_data_column].value_string.lng}</td>
                {elseif $grid_items[i][$grid_data_column].type=='checkbox'}
                    <td><input type="radio" disabled="disabled"
                               {if $grid_items[i][$grid_data_column].value==1}checked="checked"{/if}></td>
                {else}
                ...
                <td nowrap>
                    ...
                    {else}
                        {if isset($show_up_icon) && $show_up_icon}
                            <a class="btn btn-warning go_up" alt="{$grid_items[i].id.value}" href="#grow_up"><i
                                        class="icon-white icon-circle-arrow-up"></i></a>
                        {/if}
                        <a href="{$estate_folder_control}?do=edit&id={$grid_items[i].id.value}" class="btn btn-info"><i
                                    class="icon-white icon-pencil"></i></a>
                        <a onclick="return confirm('{$L_MESSAGE_REALLY_WANT_DELETE}');"
                           href="{$estate_folder_control}?{if $topic_id != ''}topic_id={$topic_id}&{/if}do=delete&id={$grid_items[i].id.value}"
                        ...
                        {if isset($grid_items[i].status_id)}
                            {if intval($grid_items[i].status_id.value)===1}
                                <a href="{$estate_folder_control}?action=data&do=set_status&status_id={$smarty.request.status_id}&page={$smarty.request.page}&set_status_id=2&id={$grid_items[i].id.value}"
                                   class="btn btn-minier btn-purple" title="На прозвон">
                                    <i class="icon-refresh"></i>
                                </a>
                            {elseif intval($grid_items[i].status_id.value)===2}
                                <a href="{$estate_folder_control}?action=data&do=set_status&status_id={$smarty.request.status_id}&page={$smarty.request.page}&set_status_id=1&id={$grid_items[i].id.value}"
                                   class="btn btn-minier btn-success" title="Дозвонились">
                                    <i class="glyphicon glyphicon-phone-alt"></i>
                                </a>
                                <a href="{$estate_folder_control}?action=data&do=set_status&status_id={$smarty.request.status_id}&page={$smarty.request.page}&set_status_id=3&id={$grid_items[i].id.value}"
                                   class="btn btn-minier btn-pink" title="Не дозвонились">
    ...
    <tr>
        <td colspan="{2+$grid_data_columns|count}">
            ...
            <button alt="data" class="batch_update btn btn-inverse"><i class="icon-white icon-th"></i> Пакетная
                обработка <sup>(beta)</sup></button>
            <button alt="data" class="duplicate btn btn-inverse"><i class="icon-white icon-th"></i> Дублировать <sup>(beta)</sup>
            </button>
        </td>
    </tr>
    ...
    {if $pager != ''}
        <tr>
            <td colspan="{2+$grid_data_columns|count}" class="pager">
